class Controller
  ...
  def edit_route
    if self.routes.empty?
      return puts "Ошибка. Необходимо сначала создать маршрут."
...
def remove_wagon
  ...
  puts "Выберите поезд:"
  self.trains.each_with_index do |train, id|
    puts "#{id}: #{train} | #{train.number} | Wagons quantity: #{train.wagons.count}"
  end
  train = self.trains[gets.chomp.to_i]

  return puts "Ошибка. У этого поезда нет вагонов." if train.wagons.empty?

  puts "Выберите вагон:"
  train.wagons.each_with_index do |wagon, id|
    puts "#{id}: #{wagon}"
  end
